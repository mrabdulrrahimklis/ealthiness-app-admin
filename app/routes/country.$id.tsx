...
import { 
  ArrowLeft, Globe, Mail, Users, Building, MapPin, Calendar, 
  TrendingUp, Activity, Award, BarChart3, Eye, Edit, Save, X, Upload
} from 'lucide-react';
...
import { useCountryDetails, useUpdateCountry, useRegions } from "~/hooks/useAuthApi";
...
export default function CountryDetailPage({ loaderData }: Route.ComponentProps) {
  ...
  const { data: apiCountry, isLoading: isLoadingCountry, error: countryError } = useCountryDetails(actualCountryId);
  ...
  const handleSave = async () => {
    try {
      ...
      if (editForm.regionId !== apiCountry?.regionId) updateData.regionId = editForm.regionId;
      if (editForm.alpha2 !== apiCountry?.alpha2) updateData.alpha2 = editForm.alpha2;
      if (editForm.alpha3 !== apiCountry?.alpha3) updateData.alpha3 = editForm.alpha3;
  ...
  const transformApiCountryToDetails = (apiCountry: ApiCountry) => ({
    ...
    totalUsers: 2020, // Mock data
    totalCompanies: 78, // Mock data
    monthlyGrowth: '+12%', // Mock data
    healthScore: 87 // Mock data
  });
        ...
        <div className="min-h-screen bg-[#F8F9FB] font-sans flex">
          ...
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center">
              <h2 className="text-2xl font-bold text-red-600 mb-2">Country Not Found</h2>
              <p className="text-gray-600 mb-4">The country you're looking for could not be found.</p>
            ...
            <div className="animate-in fade-in slide-in-from-right-8 duration-500 pb-12">
              ...
              <div className="bg-[#1B173A] rounded-[32px] p-8 text-white shadow-2xl mb-8 relative overflow-hidden flex flex-col md:flex-row items-center md:items-start gap-8 border border-[#38383A]">
                ...
                <div className="relative z-10 w-28 h-28 rounded-3xl bg-gradient-to-br from-[#5850DE] to-[#248FEC] flex items-center justify-center shadow-[0_0_40px_rgba(88,80,222,0.5)] border-4 border-[#1B173A]">
                  {flagPreview ? (
                    ...
                    <div className="text-4xl font-extrabold">{country.code}</div>
                  )}
                  {isEditing && canEdit && (
                    <button
                ...
                <div className="relative z-10 text-center md:text-left flex-1">
                  <div className="flex flex-col md:flex-row md:items-center gap-4 mb-2">
                    <div className="flex items-center gap-3 justify-center md:justify-start">
                      <h2 className="text-4xl font-extrabold">{country.name}</h2>
                  ...
                  <p className="text-[#8E8E93] font-medium mb-6 text-lg">
                    {country.code} • {country.alpha3} • Created {country.createdAt}
                  </p>
                  
                  <div className="flex flex-wrap justify-center md:justify-start gap-6">
                    <div><p className="text-[10px] text-[#8E8E93] uppercase font-bold">Total Users</p><p className="font-bold text-xl">{country.totalUsers.toLocaleString()}</p></div>
                    <div><p className="text-[10px] text-[#8E8E93] uppercase font-bold">Companies</p><p className="font-bold text-xl">{country.totalCompanies}</p></div>
                    <div><p className="text-[10px] text-[#8E8E93] uppercase font-bold">Admins</p><p className="font-bold text-xl">{country.adminCount}</p></div>
                    <div><p className="text-[10px] text-[#8E8E93] uppercase font-bold">Health Score</p><p className="font-bold text-xl text-[#248FEC]">{country.healthScore}/100</p></div>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                  ...
                  <Card className="p-8 bg-gradient-to-br from-[#F8F9FB] to-white border-[#E8E6FC]">
                    <div className="flex justify-between items-start mb-4">
                      <h3 className="text-xl font-extrabold text-[#1B173A] flex items-center gap-3">
                        ...
                        {isEditing ? 'Edit Country Information' : 'Country Administration Notes'}
                      </h3>
                    </div>
                      ...
                      <div className="space-y-6">
                        ...
                        <div>
                          ...
                          <select
                            ...
                            onChange={(e) => setEditForm(prev => ({ ...prev, regionId: e.target.value }))}
                            className="w-full px-3 py-2 border border-[#E0E1E6] rounded-lg focus:border-[#5850DE] outline-none bg-white"
                          >
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            ...
                            <Input
                              ...
                              onChange={(e) => setEditForm(prev => ({ ...prev, alpha2: e.target.value.toUpperCase() }))}
                              placeholder="US"
                              maxLength={2}
                          ...
                          <div>
                            ...
                            <Input
                              ...
                              onChange={(e) => setEditForm(prev => ({ ...prev, alpha3: e.target.value.toUpperCase() }))}
                              placeholder="USA"
                              maxLength={3}
                              className="w-full uppercase"
                            />
                            ...
                            <div className="flex items-start gap-4">
                              <div className="flex-1">
                                ...
                                <p className="text-xs text-green-600 mt-1">
                                  This will replace the current flag when you save.
                                </p>
                              </div>
                              {flagPreview && (
                    ...
                    <div className="h-64">
                      <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={COUNTRY_STATS.userGrowth}>
                          ...
                          <XAxis dataKey="month" tick={{ fontSize: 12, fill: '#8E8E93' }} />
                      ...
                      {COUNTRY_STATS.topCompanies.map((company, idx) => (
                        <div key={idx} className="flex items-center justify-between p-4 bg-white border border-[#E0E1E6] rounded-2xl hover:border-[#5850DE] transition-colors group cursor-pointer">
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <h4 className="font-bold text-[#1B173A] text-lg">{company.name}</h4>
                              <p className="text-sm font-medium text-[#8E8E93]">{company.users} active users</p>
                            </div>
                          </div>
                ...
                <div className="lg:col-span-4 space-y-8">
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center justify-between p-3 bg-[#F8F9FB] rounded-xl border border-[#E0E1E6]">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="font-medium text-[#1B173A]">Total Users</span>
                        </div>
                        <span className="font-bold text-[#1B173A]">{country.totalUsers.toLocaleString()}</span>
                      </div>
                      
                      <div className="flex items-center justify-between p-3 bg-[#F8F9FB] rounded-xl border border-[#E0E1E6]">
                        <div className="flex items-center gap-3">
                          <Building className="text-[#248FEC]" size={20} />
                          <span className="font-medium text-[#1B173A]">Companies</span>
                        </div>
                        <span className="font-bold text-[#1B173A]">{country.totalCompanies}</span>
                      </div>
                      
                      <div className="flex items-center justify-between p-3 bg-[#F8F9FB] rounded-xl border border-[#E0E1E6]">
                        <div className="flex items-center gap-3">
                          <Award className="text-[#FFB900]" size={20} />
                          <span className="font-medium text-[#1B173A]">Admins</span>
                        </div>
                        <span className="font-bold text-[#1B173A]">{country.adminCount}</span>
                      </div>
                      
                      <div className="flex items-center justify-between p-3 bg-[#F8F9FB] rounded-xl border border-[#E0E1E6]">
                        <div className="flex items-center gap-3">
                          <Activity className="text-[#4DAB46]" size={20} />
                          <span className="font-medium text-[#1B173A]">Health Score</span>
                        </div>
                        <span className="font-bold text-[#4DAB46]">{country.healthScore}/100</span>
                      </div>
                    </div>
                  </Card>
                      ...
                      {COUNTRY_STATS.demographics.map((item, idx) => (
                        <div key={idx} className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            ...
                            ></div>
                            <span className="text-sm font-medium text-[#1B173A]">{item.name}</span>
                          </div>
                          <span className="text-sm font-bold text-[#8E8E93]">{item.value}%</span>
                        </div>
                      ))}
                    </div>
                  </Card>
                  ...
                  {/* Action Buttons */}
                  <Card className="p-6">
                    <h3 className="text-lg font-extrabold text-[#1B173A] mb-4">Quick Actions</h3>
