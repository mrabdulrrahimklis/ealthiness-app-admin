...
export default function CompaniesPage() {
  ...
  const { data: companiesResponse, isLoading, error, refetch } = useCompanies({
    page: currentPage,
    limit: 10,
  ...
  const visibleCompanies = companiesResponse?.results?.map(transformCompany) || [];
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <div className="flex gap-2">
                <div className="relative" ref={dropdownRef}>
                  ...
                  >
                    {sortOptions.find(option => option.value === orderBy)?.label || "Sort by"}
                    <ChevronDown size={16} />
                  </Button>
                  {isDropdownOpen && (
                    <div className="absolute top-full left-0 mt-2 w-full bg-white border border-[#E0E1E6] rounded-lg shadow-lg z-10">
                      {sortOptions.map((option) => (
                        <button
                          key={option.value}
                          onClick={() => {
                            setOrderBy(option.value as "name" | "createdAt");
                            setIsDropdownOpen(false);
                          }}
                        ...
                        >
                          {option.label}
                          {orderBy === option.value && <Check size={16} className="text-[#5850DE]" />}
                        </button>
                      ))}
                    </div>
                  )}
                ...
                <Button
                  variant="outline"
                  onClick={() => setSortType(sortType === "ascending" ? "descending" : "ascending")}
                  className="px-3"
                ...
                {visibleCompanies.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan={6} className="p-8 text-center text-[#8E8E93]">
                      {isLoading ? "Loading..." : error ? "Error loading companies" : "No companies found"}
                    </td>
                  </tr>
                )}
              </tbody>
          ...
          {companiesResponse && companiesResponse.pages > 1 && (
            <div className="flex items-center justify-between px-6 py-4 border-t border-[#E0E1E6]">
              <div className="text-sm text-[#8E8E93]">
                Page {currentPage} of {companiesResponse.pages} ({companiesResponse.total} total)
              </div>
              <div className="flex gap-2">
                <Button
                  variant="outline"
